...
async def list_command_jobs(
    command_filter: Annotated[
        str | None, Query(description="Filter by command name")
    ] = None,
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list command jobs: {e!s}"
        )
...
async def cancel_command_job(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to cancel command job: {e!s}"
        )
