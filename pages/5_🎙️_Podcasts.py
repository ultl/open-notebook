...
def confirm_delete_speaker_profile(profile_id, profile_name) -> None:
  ...
  if usage_count > 0:
    ...
    st.write(f'This speaker profile is currently used by **{usage_count} episode profile(s)**.')
    ...
    using_episodes = [
      episode.get('name', 'Unknown') for episode in episode_profiles if episode.get('speaker_config') == profile_name
    ]
...
def speaker_configuration_dialog(mode='create', profile_id=None, episode_context=None) -> None:
  ...
  if mode == 'create':
    ...
    if episode_context:
      episode_profiles = fetch_episode_profiles()
      current_episode = next((ep for ep in episode_profiles if ep['id'] == episode_context), None)
      if current_episode:
        st.info(f'Configuring speaker for episode profile: **{current_episode.get("name", "Unknown")}**')
    ...
    if selected_speaker:
      selected_profile = next((sp for sp in speaker_profiles if sp['name'] == selected_speaker), None)
      if selected_profile:
        with st.expander(f'🎤 Preview: {selected_speaker}', expanded=True):
          st.write(f'**Description:** {selected_profile.get("description", "N/A")}')
          st.write(f'**TTS:** {selected_profile.get("tts_provider", "N/A")}/{selected_profile.get("tts_model", "N/A")}')
        ...
        if episode_context and selected_speaker:
          ...
          if current_episode:
            ...
            updated_data = {
              ...
              'outline_provider': current_episode.get('outline_provider', ''),
              'outline_model': current_episode.get('outline_model', ''),
              'transcript_provider': current_episode.get('transcript_provider', ''),
              'transcript_model': current_episode.get('transcript_model', ''),
              'default_briefing': current_episode.get('default_briefing', ''),
              'num_segments': current_episode.get('num_segments', 5),
            }

            success = update_episode_profile(episode_context, updated_data)
            if success:
              st.success(f"Speaker '{selected_speaker}' assigned to episode!")
              st.rerun()
            else:
  ...
  with col1:
    tts_provider = st.selectbox('TTS Provider*', list(tts_provider_models.keys()), key='dialog_tts_provider')

  with col2:
    tts_model = st.selectbox('TTS Model*', tts_provider_models[tts_provider], key='dialog_tts_model')
        ...
        if sp_name and valid_speakers:
          ...
          if mode == 'create':
            ...
            if success:
              ...
              pending_episode = st.session_state.get('pending_episode_assignment')
              if pending_episode:
                episode_profiles = fetch_episode_profiles()
                current_episode = next(
                  (ep for ep in episode_profiles if ep['id'] == pending_episode),
                  None,
                )
                if current_episode:
                  ...
                  assign_success = update_episode_profile(pending_episode, updated_data)
                  if assign_success:
                    st.success(f"Speaker '{sp_name}' automatically assigned to episode!")
      ...
      with st.container(border=True):
        ...
        with col_actions:
          if st.button(
            ...
            speaker_configuration_dialog('select', episode_context=profile['id'])
        ...
        if st.session_state.get(f'edit_episode_{profile["id"]}', False):
          ...
          with col5:
            ...
            outline_idx = (
              list(transcript_provider_models.keys()).index(current_outline_provider)
          ...
          with col6:
            ...
            transcript_idx = (
              list(transcript_provider_models.keys()).index(current_transcript_provider)
            ...
            if current_transcript_model in transcript_provider_models[edit_transcript_provider]:
              transcript_model_idx = transcript_provider_models[edit_transcript_provider].index(
...
def render_speaker_profiles_sidebar() -> None:
  ...
  for profile in speaker_profiles:
    ...
    with st.expander(f'🎤 {profile_name} {usage_indicator}', expanded=False):
      ...
      st.caption(f'**TTS:** {profile.get("tts_provider", "N/A")}/{profile.get("tts_model", "N/A")}')

      speakers = profile.get('speakers', [])
      # st.write(f"**Speakers:** {len(speakers)}")
      for speaker in speakers:  # Show first 2 speakers only
        st.markdown(f'- {speaker.get("name", "Unknown")} ({speaker.get("voice_id", "N/A")})\n')
      # if len(speakers) > 2:
...
with episodes_tab:
  ...
  existing_episodes = asyncio.run(repo_query('select count() from podcast_episode group all'))
  existing_episodes_count = existing_episodes[0]['count']
  if existing_episodes_count > 0:
    st.warning(f'**Please Decide:** Found {existing_episodes_count} episode(s) from the old podcast implementation.')
    c1, c2 = st.columns(2)
...
with templates_tab:
  ...
  if old_profiles:
    ...
    st.write('When you are done creating your new profiles, you can safely delete the old ones')
