              ...
              with st.spinner('Starting podcast generation...'):
                ...
                async def generate_podcast():
                  return await PodcastService.submit_generation_job(
                    ...
                    speaker_profile_name=selected_profile_obj.speaker_config if selected_profile_obj else '',
                    episode_name=episode_name.strip(),
                    content=str(context),
                    briefing_suffix=instructions.strip() or None,
  ...
  with chat_tab:
    with st.expander(f'**Session:** {current_session.title} - {humanize.naturaltime(current_session.updated)}'):
      ...
      if st.button('Create New Session', key='create_new_session'):
        new_session = create_session_for_notebook(notebook_id=current_notebook.id, session_name=new_session_name)
