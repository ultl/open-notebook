...
def notebook_header(current_notebook: Notebook) -> None:
  ...
  with st.expander(current_notebook.description if len(current_description) > 0 else 'click to add a description'):
...
def notebook_list_item(notebook) -> None:
  with st.container(border=True):
    ...
    st.caption(f'Created: {naturaltime(notebook.created)}, updated: {naturaltime(notebook.updated)}')
...
with st.expander('➕ **New Notebook**'):
  ...
  if st.button('Create a new Notebook', icon='➕'):
    notebook = notebook_service.create_notebook(name=new_notebook_title, description=new_notebook_description)
